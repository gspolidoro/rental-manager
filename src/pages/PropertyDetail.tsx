...
import { ArrowLeft, Plus, TrendingUp, TrendingDown, DollarSign, Pencil, Trash2 } from 'lucide-react'
import { getProperty, getTransactions, getCategories, createTransaction, updateTransaction, deleteTransaction } from '../lib/api'
...
export default function PropertyDetail() {
  ...
  async function handleSave(data: Omit<Transaction, 'id' | 'user_id' | 'created_at' | 'updated_at' | 'category' | 'property'>) {
    if (editing) {
      const updated = await updateTransaction(editing.id, data)
      setTransactions(prev => prev.map(t => t.id === updated.id ? updated : t))
    } else {
      const created = await createTransaction({ ...data, property_id: id! })
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className={`border rounded-xl p-4 text-center ${totalIncome - totalExpenses >= 0 ? 'bg-blue-50 border-blue-100' : 'bg-orange-50 border-orange-100'}`}>
          ...
          <p className={`text-lg font-bold ${totalIncome - totalExpenses >= 0 ? 'text-blue-700' : 'text-orange-700'}`}>{fmt(totalIncome - totalExpenses)}</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map(t => (
                <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-3 text-sm text-gray-600 whitespace-nowrap">{format(new Date(t.date), 'MMM d, yyyy')}</td>
                  <td className="px-5 py-3 text-sm text-gray-900 max-w-[200px] truncate">{t.description || '—'}</td>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-1 justify-end">
                      <button onClick={() => { setEditing(t); setShowForm(false) }} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors">
                        <Pencil className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => handleDelete(t.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors">
